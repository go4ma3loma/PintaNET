...
namespace Pinta.Core
{
	public class WorkspaceManager
	{
		...
		{
			if (HasOpenDocuments)
				PintaCore.Chrome.MainWindow.Title = string.Format ("{0}{1} - Pinta .NET", ActiveDocument.Filename, ActiveDocument.IsDirty ? "*" : "");
			else
				PintaCore.Chrome.MainWindow.Title = "Pinta .NET";
		}
		...
		private void ShowUnsupportedFormatDialog (Window parent, string filename, string primaryText, string details)
		{
			...
			string secondaryText = string.Format(Catalog.GetString("Could not open file: {0}"), filename);
			secondaryText += string.Format("\n\n{0}\n", Catalog.GetString("Pinta .NET supports the following file formats:"));
